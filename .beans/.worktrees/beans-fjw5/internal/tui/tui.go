...
func (a *App) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case launcherSelectedMsg:
		...
		if len(msg.beanIDs) == 1 {
			...
			beansDir := a.core.Root()
			beanPath := filepath.Join(beansDir, "beans-"+beanID+".md")
			cmd, result, err := launcherexec.CreateExecCommand(msg.launcher.exec, beansDir, beanID, beanPath)
			...
			// Store launcher name and result for callback
