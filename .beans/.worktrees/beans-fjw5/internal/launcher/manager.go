...
func (m *LaunchManager) startLaunch(launch *BeanLaunch, beansRoot string) error {
	// Create the command
	// Note: beansRoot parameter is actually the .beans directory (naming is confusing but kept for compatibility)
	beansDir := beansRoot
	beanPath := filepath.Join(beansDir, "beans-"+launch.Bean.ID+".md")
	cmd, result, err := CreateExecCommand(m.launcher.Exec, beansDir, launch.Bean.ID, beanPath)
	if err != nil {
