...
func CreateExecCommand(execScript, beansDir, beanID, beanTitle string) (*exec.Cmd, *ExecutionResult, error) {
	...
	// Security: Reject shebangs with dangerous characters that could lead to command injection
	// The shebang should only contain the interpreter path and arguments
	if strings.ContainsAny(shebang, ";&|$`<>()") {
		return nil, nil, fmt.Errorf("invalid shebang contains dangerous characters: %s", lines[0])
	}
